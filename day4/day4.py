...
def has_all_fields(pp_fields):
    expected = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
    fields_valid = True
    for field in expected:
        val = pp_fields.get(field, None)
        if field not in pp_fields or val is None:
            fields_valid = False
            continue
...
def format_input(input):
    ...
    for f in input:
        ...
        pp_fields = {}
        for elem in elems:
            field_and_val = elem.split(':')
            val = field_and_val[1] if len(field_and_val) > 1 else None
            pp_fields[field_and_val[0]] = val
        formatted.append(pp_fields)
    return formatted

passports = format_input(input.split('\n\n'))

num_valid = 0
for passport in passports:
    is_valid = has_all_fields(passport)
    if not is_valid:
        continue
    ...
    height = int(items[0].strip())
    unit = items[1].strip().lower()
    if unit == 'cm':
    ...
    ecl = passport['ecl']
    valid = False
    valid_opts = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
